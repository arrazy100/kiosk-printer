...
class PrinterData : Serializable {
    var id: Int? = null
    var name: String = ""
    var printerDpi: Int = 203
    ...
    fun retrieveDeviceConnection(context: Context, appCompatActivity: AppCompatActivity) {
        if (printerModule?.equals(PrinterModuleEnum.Network) == true) {
            ...
            bluetoothConnection = BluetoothPrinterPermissions.retrieveBluetoothDevice(
                macAddress,
                context,
                appCompatActivity
            )
            ...
            val usbDevice = USBPrinterPermission.retrieveUsbDevice(
                usbProductId!!,
                usbVendorId!!,
                context,
                appCompatActivity
